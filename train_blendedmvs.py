...
parser = argparse.ArgumentParser(description='AdaptMVSNet for high-resolution multi-view stereo')
parser.add_argument('--mode', default='train', help='train or val', choices=['train', 'val'])
parser.add_argument('--model', default='AdaptMVSNet', help='select model')
...
if args.dataset == 'dtu_yao':
    ...
    train_dataset = MVSDataset(args.trainpath, args.trainlist, "train", nviews=7)
    test_dataset = MVSDataset(args.valpath, args.vallist, "val", nviews=7)

TrainImgLoader = DataLoader(train_dataset, args.batch_size, shuffle=True, num_workers=8, drop_last=True)
TestImgLoader = DataLoader(test_dataset, args.batch_size, shuffle=False, num_workers=4, drop_last=False)

# model, optimizer
model = AdaptMVSNet(patchmatch_interval_scale=args.patchmatch_interval_scale,
...
#     for n in m.modules():
#         if isinstance(n, (torch.nn.Linear, torch.nn.Conv1d, torch.nn.Conv2d,torch.nn.Conv3d)):
#             torch.nn.init.xavier_uniform_(m.weight)
model.cuda()
model_loss = model.adaptmvsnet_loss()
...
compute_normals_0 = NormalGenerator(height=576,width=768)
compute_normals_1 = NormalGenerator(height=int(576/2),width=int(768/2))
compute_normals_2 = NormalGenerator(height=int(576/4),width=int(768/4))
compute_normals_3 = NormalGenerator(height=int(576/8),width=int(768/8))
...
def train_sample(sample, detailed_summary=False):
    ...
    normals_gt = {
        "stage_0": compute_normals_0(depth_gt["stage_0"], invK["stage_0"]),
        "stage_1": compute_normals_1(depth_gt["stage_1"], invK["stage_1"]),
        "stage_2": compute_normals_2(depth_gt["stage_2"], invK["stage_2"]),
        "stage_3": compute_normals_3(depth_gt["stage_3"], invK["stage_3"]),
    }
    # estimate normals for depth
    normals_pred = {
        "stage_0": compute_normals_0(depth_est["stage_0"], invK["stage_0"]),
        "stage_1": compute_normals_1(depth_patchmatch["stage_1"][0], invK["stage_1"]),
        "stage_2": compute_normals_2(depth_patchmatch["stage_2"][0], invK["stage_2"]),
        "stage_3": compute_normals_3(depth_patchmatch["stage_3"][0], invK["stage_3"]),
    }
    loss = model_loss(depth_patchmatch, depth_est, depth_gt,normals_gt,normals_pred,mask)
