...
def save_depth():
    ...
    model = AdaptMVSNet(patchmatch_interval_scale=args.patchmatch_interval_scale,
                ...
                propagate_neighbors=args.propagate_neighbors, evaluate_neighbors=args.evaluate_neighbors,n_views=args.n_views)
    model.cuda()
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            torch.cuda.synchronize(device=0)
            print('Iter {}/{}, time = {:.3f}'.format(batch_idx, len(TestImgLoader), (time.time() - start_time) * 1000))
            #-----------------------------
            #
...
def filter_depth_blend(scan_folder, out_folder, plyfilename, geo_pixel_thres, geo_depth_thres, photo_thres, img_wh):
    # the pair file
